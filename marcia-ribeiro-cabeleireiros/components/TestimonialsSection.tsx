      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <div className="flex items-center justify-center gap-2">
            <div className="flex">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="w-5 h-5 text-rose-400 fill-rose-400" />
              ))}
            </div>
          ...
          {business.testimonials.map((t, i) => (
            ...
            >
              <Card3D className="glass-panel-strong rounded-2xl p-6 flex flex-col gap-4 relative" strength={6}>
                <span className="absolute top-4 right-5 text-7xl leading-none text-rose-500/10 font-serif select-none pointer-events-none">
                  &ldquo;
                </span>
                <div className="flex">
                  {[...Array(t.rating)].map((_, j) => (
                    <Star key={j} className="w-4 h-4 text-rose-400 fill-rose-400" />
                  ))}
                </div>
                <p className="text-zinc-300 text-sm leading-relaxed flex-1">&ldquo;{t.text}&rdquo;</p>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-rose-500/20 ring-1 ring-rose-500/30 flex items-center justify-center text-rose-400 text-sm font-bold">
                      {t.name[0]}
                    </div>
